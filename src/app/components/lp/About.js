...
export default function About() {
  ...
  return (
    <section
      id="whatwedo"
      className="about relative h-[80vh] sm:h-screen bg-black bg-contain bg-no-repeat bg-left-top bg-[url('/images/about-bg.png')]"
    >
      <div className="w-full container px-4">
        <div className="flex w-80 sm:w-96 mx-auto gap-12 h-screen relative -top-0 sm:-top-28">
        {sections.map(({ id, title }, index) => (
            <div
              key={id}
              className={`w-24 bg-primary h-[30%] sm:h-2/5 xl:h-2/4 flex items-end justify-end cursor-pointer transition-all duration-300 about-sharps relative after:content-[''] after:bg-primary after:h-[100px] after:w-[4.67rem] sm:after:w-[6.02rem] after:absolute after:left-0 after:clip-about-sharp ${index === 0 ? 'after:-top-8' : index === 1 ? 'after:-top-16' : 'after:-top-24'}  ${activeSection === id ? '!h-1/3 sm:!h-full' : ''}`}
        ...
        <div className="absolute inset-0 flex items-center justify-center h-min top-[45%] sm:top-[34%] lg:top-[38%]">
          <div
        ...
        <div className="absolute top-[50%] sm:top-auto sm:bottom-24 left-0 sm:left-1/2 sm:transform sm:-translate-x-1/2 text-white text-left">
          ...
          {activeSection === 'seo' && (
            <div id="seo-content" className="relative p-7 sm:p-0 sm:left-[14rem] sm:w-80">
              ...
              <button className="mt-5 group rounded-full bg-primary px-2 py-1 text-base font-semibold text-white transition-colors hover:bg-neutral-700 focus:outline-none focus:bg-neutral-700">
                Get Started
              </button>
            </div>
          )}
